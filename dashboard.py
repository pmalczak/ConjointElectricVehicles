...
from analyze import read_hierarchical_df, create_augmented_products_df, interpolateUtility
from analyze import simulatorOutput, OfferMatching, InFlowsOutFlowsBase, Simulate, InFlowsOutFlows
from textwrap import dedent as d
import plotly.graph_objs as go
import plotly.figure_factory as ff

from plot_finacials import plot_financials
...
def sweep_calculate(sweepSpecify, attribute, sweepType="Lockstep"):
    ...
    if (sweepType == "Attribute Grid"):
        ...
        for i in range(upfrntGridN):
            for j in range(mnthlyGridN):
                ...
                augmentProductsDf = create_augmented_products_df(useProductsDf, dictCatAttributes)
...
def analysis_options(type_analysis):
    if type_analysis == "Lockstep":
    ...
    elif type_analysis == "Attribute Grid":
...
def calculateOffer(nclicks, attribute, rows, columns, sweepType):
    ...
    if nclicks is None:
        ...
        if sweepType == "Lockstep":
            returnDf, dfFlow = sweep_calculate(df, attribute, sweepType)
            augVar = dfFlow.to_json(date_format='iso', orient='split')
        elif sweepType == "Attribute Grid":
            returnDf, baseVar = sweep_calculate(df, attribute, sweepType)
            augVar = baseVar
...
def display_output_Lockstep(jsonified_data, provider, baseInds, jsonified_flow, viewType, val, attribute, analysisType):
    if jsonified_flow is None:
        ...
        figData = plot_financials(returnDf, dfFlow, provider, attribute, val, viewType, aggView, baseInds)
...
def display_grid(jsonified_data, baseVar, viewType):
    if jsonified_data is None:
        ...
        if viewType == "Nominal":
            ...
            ebit_title = "EBIT Change ($ thousands per month)"
            ...
            ebit_title = "EBIT Relative Change (Percentage)"
